...
const GUNLUK_TIKETIMLER = [
  ...
  { id: 'kagit', label: 'Kağıt kullanımı', icon: '📄', litre: 10, options: [{ label: 'Çok fazla', mul: 10 }, { label: 'Orta', mul: 5 }, { label: 'Az', mul: 2 }] },
...
export default function SuAyakIziPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--ocean-deep)' }}>
      ...
      {/* Hero */}
      <section style={{ padding: '70px 24px 50px', textAlign: 'center', borderBottom: '1px solid rgba(52,152,219,0.15)', background: 'radial-gradient(ellipse at 50% 80%, rgba(52,152,219,0.08) 0%, transparent 60%)' }}>
        <div style={{ maxWidth: 680, margin: '0 auto' }}>
          <div style={{ display: 'inline-block', background: 'rgba(52,152,219,0.15)', border: '1px solid rgba(52,152,219,0.35)', borderRadius: 20, padding: '5px 16px', fontSize: 11, fontWeight: 700, color: '#85c1e9', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 18 }}>Araştırma Sorusu 6</div>
          ...
          <p style={{ fontSize: 16, color: 'rgba(202,240,248,0.68)', lineHeight: 1.8, marginBottom: 20 }}>
            Deniz Kirliliğiyle Mücadele Kiti araştırmasında en az bilinen kavramlardan biri olan su ayak izi nedir? Kendi su ayak izini hesapla.
          </p>
          <div style={{ background: 'rgba(52,152,219,0.1)', border: '1px solid rgba(52,152,219,0.22)', borderRadius: 12, padding: '16px 20px', maxWidth: 560, margin: '0 auto', textAlign: 'left' }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: '#85c1e9', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 7 }}>📋 Araştırma Sorusu</div>
            <p style={{ fontSize: 15, color: 'rgba(202,240,248,0.8)', fontStyle: 'italic' }}>"Su ayak izi ne olabilir?"</p>
          </div>
        </div>
      </section>
      ...
      <section style={{ padding: '55px 24px', background: 'rgba(13,33,55,0.7)' }}>
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(1.3rem,2.5vw,1.9rem)', color: 'var(--ocean-surface)', marginBottom: 28, textAlign: 'center' }}>
            Katılımcıların Cevapları
          </h2>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
            <div>
              <div style={{ fontSize: 12, fontWeight: 700, color: '#e8a87c', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 14 }}>Ön Test (Kit Öncesi)</div>
              {ONTEST.map((k, i) => (
            ...
            <div>
              <div style={{ fontSize: 12, fontWeight: 700, color: '#74c69d', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 14 }}>Son Test (Kit Sonrası)</div>
              {SONTEST.map((k, i) => (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: 14 }}>
            {URUN_AYAK_IZI.map((u, i) => (
              <div key={i} className="card-ocean" style={{ padding: '18px 16px', textAlign: 'center' }}>
                ...
                <div style={{ fontFamily: 'var(--font-display)', fontSize: '1.5rem', color: 'var(--ocean-bright)', fontWeight: 700 }}>
        ...
        <div style={{ maxWidth: 700, margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
            {GUNLUK_TIKETIMLER.map(item => (
              <div key={item.id} style={{ background: 'rgba(14,61,92,0.4)', border: '1px solid rgba(0,180,216,0.18)', borderRadius: 12, padding: '18px 20px' }}>
                <p style={{ fontSize: 15, color: 'rgba(202,240,248,0.82)', marginBottom: 12 }}>
          ...
          {hepsSecildi && (
            <div style={{ marginTop: 28, background: 'rgba(0,180,216,0.12)', border: '1px solid rgba(0,180,216,0.35)', borderRadius: 14, padding: '24px', textAlign: 'center' }}>
              <p style={{ fontSize: 14, color: 'rgba(202,240,248,0.6)', marginBottom: 8 }}>Tahmini günlük su ayak izin</p>
              <div style={{ fontFamily: 'var(--font-display)', fontSize: '3rem', color: 'var(--ocean-bright)', fontWeight: 700, marginBottom: 8 }}>
